...
description: Apply this structured Conventional Commits variant when writing or reviewing a commit message subject — the 11 standard types, one monochrome geometric-shape marker per type and a shared cross-project scope vocabulary (reference/scopes.md). Use whenever generating a commit message, validating one for compliance, or when another skill (e.g. git-commit-no-ai, commit-history-rewrite) needs the canonical format definition. Never invent a scope not in reference/scopes.md without adding it there first.
---

# commit-convention

A structured, cross-project Conventional Commits variant: the same grammar as the spec, plus a
fixed shape marker per type and a shared scope vocabulary — so commit
...
The colon follows `<type>(<scope>)` directly, with no padding. `<scope>` is optional. Every
space in the format (after the colon, around the ` - `) is a single space, never a run.
...
single area fits. Max 8 characters.
...
```text
refactor(router): ◆ - extract route matching into a helper
```
